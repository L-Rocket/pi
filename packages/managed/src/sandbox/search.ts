...
export async function grepSearch(request: GrepRequest, searchPath: string): Promise<GrepResponse> {
	...
	const matches: GrepMatch[] = context === 0 ? rawMatches : await attachContext(rawMatches, context);
	return { matches, truncated };
}

async function attachContext(rawMatches: GrepMatch[], context: number): Promise<GrepMatch[]> {
...
export async function findFiles(request: FindRequest, searchPath: string, insideGitRepo: boolean): Promise<FindResponse> {
